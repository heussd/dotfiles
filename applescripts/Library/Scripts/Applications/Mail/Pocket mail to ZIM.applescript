	...
	tell application "Mail"
		...
		if (count of selectedMessages) is equal to 0 then
			...
			repeat with theMessage in selectedMessages
				...
				set theNotes to theNotes & my asQuote(my getPocketText(theDecodedZettel), (my getPocketSource(theDecodedZettel))) & my newline() & my newline() & my newline()
				
			end repeat
			my writeFile(my getPocketSource(theDecodedZettel), my addZIMMeta(theNotes))
...
on asQuote(content, source)
	return "'''" & my removeJunk(content) & my newline() & "'''" & my newline() & my newline() & my replaceText(my newline() & my newline(), my newline(), my removeJunk(source)) & (do shell script "date")
end asQuote
...
on getPocketSource(decodedPocketMailSource)
	set AppleScript's text item delimiters to "Quote from the article:"
	return (text of text item 1 of decodedPocketMailSource)
end getPocketSource
